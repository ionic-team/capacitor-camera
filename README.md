...
| **`thumbnail`** | <code>string</code>                                     | Returns the thumbnail of the media, base64 encoded. On Web, for <a href="#mediatype">`MediaType.Photo`</a>, the full image is returned here, also base64 encoded. On Web, for <a href="#mediatype">`MediaType.Video`</a>, a JPEG thumbnail captured from the video is returned. | 8.1.0 |
...
| **`creationDate`** | <code>string</code> | The date and time the media was created, in ISO 8601 format. If creation date is not available (e.g. Android 7 and below), the last modified date is returned.                                                                               | 8.1.0 |
...
| **`limit`**                  | <code>number</code>                                               | The maximum number of media files that the user can choose. Only applicable if `allowMultipleSelection` is `true`. Any non-positive number will be treated as unlimited. Note: This option is only supported on Android 13+ and iOS. Not available on Web.                                                     | <code>0</code>                        | 8.1.0 |
| **`includeMetadata`**        | <code>boolean</code>                                              | Whether or not <a href="#mediaresult">MediaResult</a> should include its metadata. If an error occurs when obtaining the metadata, it will return empty. Note: This option is only supported on Android and iOS.                                                                                               | <code>false</code>                    | 8.1.0 |
